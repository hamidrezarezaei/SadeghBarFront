...
import { GetList_New_Cargo_Api, GetList_New_GreaterThanId_Cargo_Api } from '../../Api/cargoApi';
import { useToast } from "react-native-toast-notifications";
import Loading from '../../Component/Loading/Loading';
import { globalStyles } from '../../assets/Styles/GlobalStyle';
import { newCargoListStyles } from './NewCargoListStyle';
...
// =================================================================
export default function NewCargoListScreen(props) {
  ...
  const loadData = async (pNumber = 1) => {
    try {
      ...
      let data = await GetList_New_Cargo_Api(searchParams);
  ...
  const refreshScreen = async () => {
    try {
      ...
      let data = await GetList_New_GreaterThanId_Cargo_Api(searchParams);
      if (data.messageStatus == "Successful") {
        if (data.messageData.data && data.messageData.data.length > 0) {
  ...
  return (
    <View style={{ flex: 1 }} nestedScrollEnabled={true} >
      <View style={[globalStyles.screenContainer, newCargoListStyles.screenContainer]} >
        ...
        <View style={newCargoListStyles.cargoesContainer}>
          <FlatList
            data={cargoes}
